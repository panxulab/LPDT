    ...
    def __init__(self, n_tasks: int = 20, tasks: List = None) -> None:
        ...
        if tasks is None:
            ...
            for i in range(n_tasks):
                env = reach_env.train_classes['pick-place-v2']()
...
class PickPlaceEnv:
    def __init__(self, n_tasks: int = 20, task_list: List = None) -> None:
        self.env_list = []
        self._max_episode_step = 500
        
        pick_place_env = metaworld.ML1('pick-place-v2', 3407)
        if task_list is None:
            self.n_tasks = n_tasks
            self.tasks = []
            for i in range(n_tasks):
                env = pick_place_env.train_classes['pick-place-v2']()
                env.max_path_length = self._max_episode_step
                task = pick_place_env.train_tasks[i]
                env.set_task(task)
                self.env_list.append(env)
                self.tasks.append(task)
        else:
