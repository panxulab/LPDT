...
class Reinformer(TrajectoryModel):
    """
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None, past_key_values=None, prompt=None):
        ...
        if prompt is not None:
            ...
            prompt_stacked_inputs = torch.stack(
                (prompt_state_embeddings, prompt_returns_embeddings, prompt_action_embeddings), dim=1
            ...
            if self.reconstruction:
                logits = self.reconstruction_head(prompt_states, prompt_actions, torch.mean(prompt_stacked_inputs, dim=1))
        ...
        return_preds = self.predict_return(x[:, 0])[:, -seq_length:, :]
    ...
    def get_return(self, states, actions, rewards, returns_to_go, timesteps, past_key_values=None, prompt=None, **kwargs):
            # we don't care about the past rewards in this model
            states = states.reshape(1, -1, self.state_dim)
            actions = actions.reshape(1, -1, self.act_dim)
            returns_to_go = returns_to_go.reshape(1, -1, 1)
