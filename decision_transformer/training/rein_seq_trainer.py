...
class ReinSequenceTrainer(Trainer):
    def pure_train_iteration_mix(self, num_steps, no_prompt=False):
        ...
        for _ in range(num_steps):
            train_loss, action_loss, return_loss, reg_loss = self.train_step_mix(no_prompt=no_prompt)
        ...
        logs['training/return_loss_mean'] = np.mean(return_losses)
        logs['training/return_loss_std'] = np.std(return_losses)
    ...
    def train_step_mix(self, no_prompt=False):
        ...
        return_target = torch.clone(rtg[:, 1:])
        num_samples = states.shape[0]

        if no_prompt:
            state_preds, action_preds, return_preds, logits = self.model.forward(
        ...
        return_target = return_target.reshape(-1, 1)[attention_mask.reshape(-1) > 0]

        norm = return_target.abs().mean()
        u = (return_target - return_preds) / norm
        returns_loss = torch.mean(
            torch.abs(
                0.99 - (u < 0).float()
            ) * u ** 2
        )
        ...
        if self.args["infoNCE"]:
            ...
            reg_loss = F.mse_loss(logits, prompt_states)

            loss += self.args["reconstruction_lambda"] * reg_loss
        else:
        ...
        return loss.detach().cpu().item(), action_loss.detach().cpu().item(), returns_loss.detach().cpu().item(), reg_loss.detach().cpu().item()
    ...
    def eval_iteration_multienv(self, get_prompt, prompt_trajectories_list, eval_episodes, env_name_list, info,
        ...
        eval_start = time.time()
        for env_id, env_name in enumerate(env_name_list):
            # single-pass evaluation without best-of prompt selection
            self.eval_fns = [eval_episodes(tar, info[env_name], variant, env_list[env_id], env_name) for tar in
                            info[env_name]['env_targets']]
